...
namespace CodeAnalysis
{
  ...
  template<typename element>
  void TreeWalkReturnStr(element* pItem, std::string& parserInfo, bool details = false)
  {
	  static std::string path;
	  static size_t indentLevel = 0;
	  std::ostringstream out;
	  out << "\n  " << std::string(2 * indentLevel, ' ') << pItem->showReqDetails();
	  std::cout << out.str();
	  parserInfo.append(out.str());
	  auto iter = pItem->children_.begin();
	  ++indentLevel;

	  while (iter != pItem->children_.end())
	  {
		  TreeWalkReturnStr(*iter, parserInfo);
		  ++iter;
	  }
	  --indentLevel;
  }
